...
collectAndGenerateThreads gt st (stmt : rest) currentAddr threadCounter = 
    ...
    let stmtCode = generateStmtCode gt (newBlockInStack st) [] currentAddr stmt
    let stmtSize = length stmtCode
    let (restThreads, restCode, finalAddr) = collectAndGenerateThreads gt (newBlockInStack st) rest (currentAddr + stmtSize) threadCounter
...
generateNormalBodyWithNested gt st body nestedThreads = do
    ...
    let ownCode = concatMap (generateStmtCodeForThread gt st nestedThreads 0) body
        -- Add nested thread bodies at the end
    let nestedBodies = concatMap (\(_, _, _, nestedBody) -> 
                                   generateThreadBodyWithNested gt (fillLocalDisplayForThisBody (newBlockInStack st) nestedBody) nestedBody []) nestedThreads
